...
class Income(ctk.CTkFrame):
    ...
    def __init__(self, master, title):
        ...
        self.addButton = ctk.CTkButton(self, text="Add", font=self.FONT, command=self.add_income)
        self.addButton.grid(row=2, column=0, padx=5, pady=5, sticky="w")

        self.deleteButton = ctk.CTkButton(self, text="Delete", font=self.FONT, command=self.delete_income)
    ...
    def load_income_data(self):
        """Load income data from workbook into treeview"""
        try:
            # Clear existing items
            self.treeview.delete(*self.treeview.get_children())
            
            # Get data from workbook
            income_data = self.workbook.get_sheet_data('Income')
